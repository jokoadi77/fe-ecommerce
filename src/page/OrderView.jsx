    ...
    <tbody>
      {orders.map((item, index) => (
           ...
           {item.itemsDetail.map((itemProduct) => (
                <li key={itemProduct.product} className="mt-2">
                  <span className="">{itemProduct.name}</span> <br />
                  <span className="">Quantity: {itemProduct.quantity}</span> <br />
                  <span className="">{priceFormat(itemProduct.price)}</span>
                </li>
           ))} </ul>
           </td>
           <td className="font-bold italic">{priceFormat(item.total)}</td>
           <td>
              {
                item.status === "pending" ? (<span className="btn btn-info">Pending</span>) : 
                item.status === "success" ? (<span className="btn btn-accent">Success</span>) :
